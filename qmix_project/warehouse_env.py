...
class WarehouseEnv:
    ...
    def __init__(
        ...
        restock_delay:   int                        = RESTOCK_DELAY,
    ):
        self.H, self.W   = 16, 20
    ...
    def reset(self):
        ...
        # ── 对比实验统计（纯计数器，不影响动力学/obs/reward）──
        self.total_moves      = 0            # 累计有效移动格数 → 行驶距离/能耗
        self.conflict_count   = 0            # 让步+卡死重规划次数 → 协同冲突
        self.wait_times       = []           # 每件货物“刷新→被取走”的等待步数
        self.shelf_birth_step = [0] * self.n_shelves   # 当前货物何时上架
        ...
        for k in range(self.n_shelves):
            if self.shelf_restock_timer[k] > 0:
                ...
                if self.shelf_restock_timer[k] == 0:
                    ...
                    self.shelf_birth_step[k] = self.step_count   # 记录上架时刻
    ...
    def _move_agents(self):
        ...
        for i in range(self.n_agents):
            for j in range(i + 1, self.n_agents):
                if (intended[i] == tuple(self.agent_pos[j]) and
                    ...
                    self.conflict_count += 1   # 对头让步 → 记一次冲突
        ...
        self.total_moves += sum(moved)   # 本步有效移动格数 → 累计行驶距离
        ...
        for i in range(self.n_agents):
            ...
            if not at_dest and not self.agent_paths[i]:
                ...
                if self._stuck_count[i] >= 5:
                    ...
                    self.agent_paths[i] = new_path
                    self._stuck_count[i] = 0
                    self.conflict_count += 1   # 卡死触发重规划 → 记一次冲突
            else:
                self._stuck_count[i] = 0
        ...
        if pos in self.shelf_positions:
            ...
            if self.agent_carrying[agent_id] == GOODS_NONE:
                if self.shelf_goods[shelf_idx] != GOODS_NONE:
                    ...
                    self.shelf_restock_timer[shelf_idx] = self.restock_delay
                    self.wait_times.append(self.step_count - self.shelf_birth_step[shelf_idx])
